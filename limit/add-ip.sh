...
data_ip="https://raw.githubusercontent.com/stangkudut/ST4VIPSC/main/izinIP"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    echo -e ""
    echo -e "            ${RED}DAFTAR DULU BG !${NC}"
    echo -e "   \033[0;33mYour VPS${NC} $ipsaya \033[0;33mHas been Banned${NC}"
    echo -e "     \033[0;33mBuy access permissions for scripts${NC}"
    echo -e "             \033[0;33mContact Admin :${NC}"
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/6282273761014"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    exit
  fi
}
checking_sc
clear
Repo1="https://raw.githubusercontent.com/stangkudut/ST4VIPSC/main/"
cd izin
export MYIP=$( curl -sS ipv4.icanhazip.com/ )
SELLER=$(curl -sS ${Repo1}ip | grep $MYIP | awk '{print $2}')
Exp100=$(curl -sS ${Repo1}ip | grep $MYIP | awk '{print $3}')
data_ip="https://raw.githubusercontent.com/stangkudut/ST4VIPSC/main/izin"
cd izin
...
URL="https://api.telegram.org/bot$KEY/sendMessage"
    ...
    git clone ${REPO} /root/ST4VIPSC/ &> /dev/null
    clear
    echo -e ""
    echo -e ""
    read -p "  Input IP Address : " ip
    CLIENT_EXISTS=$(grep -w $ip /root/ST4VIPSC/izinIP | wc -l)
   ...
   # read -p "  Input Username IP (Example : Lt) : " name
    ...
    echo "### ${name} ${exp2} ${ip}" >> /root/ST4VIPSC/izinIP
    elif [[ ${exp} == '2' ]]; then
    ...
    git remote add origin https://github.com/stangkudut/ST4VIPSC
    git push -f https://${TOKEN}@github.com/stangkudut/ST4VIPSC.git &> /dev/null
    cd izinIp
    rm -rf /root/ST4VIPSC
    clear
    sleep 1
...
URL="https://api.telegram.org/bot$KEY/sendMessage"
    ...
    name=$(cat /root/ST4VIPSC/izinIP | grep $ipdel | awk '{print $2}')
    exp=$(cat /root/ST4VIPSC/izinIP | grep $ipdel | awk '{print $3}')
    if [[ ${exp} == 'Lifetime' ]]; then
    sed -i "/^#&   $name   $exp $ipdel/,/^},{/d" /root/ST4VIPSC/izinIP
    else
    sed -i "/^### $name $exp $ipdel/,/^},{/d" /root/ST4VIPSC/izinIP
    fi
    cd /root/ST4VIPSC
...
URL="https://api.telegram.org/bot$KEY/sendMessage"
    ...
    sed -i "s/### $name $exp $ipdel/### $name $exp4 $ipdel/g" /root/ST4VIPSC/izinIP
    cd /root/ST4VIPSC
    git config --global user.email "${EMAIL}" &> /dev/null
    git config --global user.name "${USER}" &> /dev/null
    rm -rf .git &> /dev/null
